...
export async function POST(request: Request) {
	...
	if (attendees.length && input.mailboxId) {
		...
		await Promise.all(attendees.map((to) => sendEmail(env, { userId: user.id, mailboxId: input.mailboxId!, from: input.from ?? "", to, subject: `دعوة: ${event.title}`, text: event.description || `أنت مدعو إلى ${event.title}.`, attachments: [{ filename: "invite.ics", type: "text/calendar; charset=utf-8", content: calendarFile }] })));
	}
