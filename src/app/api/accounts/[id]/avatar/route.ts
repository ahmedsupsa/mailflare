...
export async function POST(request: Request, { params }: AccountRouteParams) {
	...
	if (!isUploadedAvatarFile(file)) return NextResponse.json({ error: "ملف الصورة مفقود" }, { status: 400 });
	if (!ALLOWED_AVATAR_TYPES.includes(file.type)) return NextResponse.json({ error: "استخدم صورة بصيغة JPEG أو PNG أو WebP أو GIF" }, { status: 400 });
	if (file.size > MAX_AVATAR_SIZE) return NextResponse.json({ error: "يجب ألا يتجاوز حجم الصورة 2 ميجابايت" }, { status: 413 });
