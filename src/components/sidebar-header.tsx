...
export function SidebarHeader({ href, label }: SidebarHeaderProps) {
	...
	return (
		<div className={`mb-3 flex h-10 items-center ${minimal ? "justify-center" : "gap-2 px-1"}`}>
			<button type="button" onClick={toggle} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-neutral-600 hover:bg-neutral-200" aria-label={minimal ? "توسيع القائمة" : "طي القائمة"}>
