...
The Deploy to Cloudflare flow can deploy the Worker, but its deployment token is not exposed to the app at runtime. Create a separate Cloudflare API token and set it as `CF_TOKEN`.
